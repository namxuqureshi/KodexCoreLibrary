...
open class SearchTextField: UITextField {
    ...
    open var comparisonOptions: NSString.CompareOptions = [.caseInsensitive,.literal,.backwards]
        ...
        if let firstElement = filteredResults.first {
            if let itemSelectionHandler = self.itemSelectionHandler {
                ...
                if inlineMode, let filterAfter = startFilteringAfter {
                    ...
                    self.string = stringElements!.first! + filterAfter + firstElement.title
                } else {
                    self.string = firstElement.title
                }
            }
        }
    }
    ...
    fileprivate func filter(forceShowAll addAll: Bool) {
        ...
        for i in 0 ..< filterDataSource.count {
            ...
            if !inlineMode {
                ...
                let titleFilterRange = (item.title as NSString).range(of: text!.replacingOccurrences(of: "’", with: "'").replacingOccurrences(of: "\\", with: ""), options: comparisonOptions)
                let subtitleFilterRange = item.subtitle != nil ? (item.subtitle! as NSString).range(of: text!.replacingOccurrences(of: "’", with: "'").replacingOccurrences(of: "\\", with: ""), options: comparisonOptions) : NSMakeRange(NSNotFound, 0)
                ...
                var textToFilter = text!.lowercased().replacingOccurrences(of: "’", with: "'").replacingOccurrences(of: "\\", with: "")
    ...
    fileprivate func prepareDrawTableResult() {
        let keyWindow = UIApplication.shared.windows.filter {$0.isKeyWindow}.first ?? UIWindow.init()
        guard let frame = self.superview?.convert(self.frame, to: keyWindow) else { return }
...
//    let bottomInset = CGFloat(0.0), leftInset = CGFloat(0.0), rightInset = CGFloat(0.0)
//    
...
extension SearchTextField: UITableViewDelegate, UITableViewDataSource {
    ...
    public func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        cell!.textLabel?.string = filteredResults[(indexPath as NSIndexPath).row].title
        cell!.detailTextLabel?.string = filteredResults[(indexPath as NSIndexPath).row].subtitle
    ...
    public func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        if itemSelectionHandler == nil {
            self.string = filteredResults[(indexPath as NSIndexPath).row].title
        } else {
