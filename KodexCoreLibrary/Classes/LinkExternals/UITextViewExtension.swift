...
//  The Craft
//
//  Created by J Aiden on 02/04/2019.
//  Copyright © 2019 
//
...
extension UITextView: UITextViewDelegate {
    ...
    func resizePlaceholder(addInX:CGFloat = 0,addInY:CGFloat = 0) {
        if let placeholderLabel = self.viewWithTag(100) as! UILabel? {
            let labelX = self.textContainer.lineFragmentPadding
            let labelY = self.textContainerInset.top - 2
            let labelWidth = self.frame.width - (labelX * 2)
            let labelHeight = placeholderLabel.frame.height
            placeholderLabel.frame = CGRect(x: addInX+labelX, y: addInY+labelY, width: labelWidth, height: labelHeight)
        }
    }
...
extension ExpandableLabel {
    private func commonInit() {
        ...
        collapsedAttributedLink = NSAttributedString(string: "read more", attributes: [.font: UIFont.boldSystemFont(ofSize: font.pointSize),.foregroundColor : UIColor.init(named: "AccentColor") ?? UIColor.blue])
    ...
    private func textReplaceWordWithLink(_ lineIndex: LineIndexTuple, text: NSAttributedString, linkName: NSAttributedString) -> NSAttributedString {
        ...
        (lineText.string as NSString).enumerateSubstrings(in: NSRange(location: 0, length: lineText.length), options: [.byWords, .reverse]) { (word, subRange, enclosingRange, stop) -> Void in
            ...
            if let ellipsis = self.ellipsis {
                ...
                lineTextWithAddedLink.append(NSAttributedString(string: " ", attributes: [.font: self.font ?? UIFont.systemFont(ofSize: CGFloat(12))]))
            }
    ...
    private func textReplaceWithLink(_ lineIndex: LineIndexTuple, text: NSAttributedString, linkName: NSAttributedString) -> NSAttributedString {
        ...
        if let ellipsis = self.ellipsis {
            ...
            linkText.append(NSAttributedString(string: " ", attributes: [.font: self.font ?? UIFont.systemFont(ofSize: CGFloat(12))]))
        }
